...
function MedicineInstructionsEditor({
  prescriptionId,
  medicine,
  onSaved,
}: {
  ...
  const save = async () => {
    ...
    try {
      const res = await fetch(`/api/prescriptions/${prescriptionId}`, {
        ...
        body: JSON.stringify({
          medicines: [
            { id: medicine.id, timing, foodInstructions, usageInstructions },
          ],
        }),
      });
  ...
  if (!editing) {
    return (
      <div className="flex items-center gap-2">
        ...
        >
          {timing || foodInstructions || usageInstructions ? "Edit" : "Add"}{" "}
          timing / food / usage instructions
        </Button>
      </div>
    );
  }
...
export default function PatientDetailPage() {
  ...
  const generateBill = async (consultationId: string) => {
    ...
    try {
      ...
      toast(
        `Invoice ${data.invoiceNumber} created — ₹${data.total}`,
        "success",
      );
      fetchPatient();
    } catch (err: any) {
  ...
  return (
    <div className="p-8">
          ...
          <div className="grid grid-cols-1 md:grid-cols-2 lg:grid-cols-3 gap-6">
            ...
            <div>
              ...
              <Badge variant="outline">
                {bloodGroupDisplay[patient.bloodGroup || ""] || "N/A"}
              </Badge>
            </div>
            <div>
              <p className="text-sm text-gray-600 flex items-center gap-2">
      ...
      <div className="grid grid-cols-1 md:grid-cols-3 gap-4 mb-6">
        ...
        {isDoctor && inProgressAppointment && (
          <Link href={`/opd/consultation/${inProgressAppointment.id}`}>
            <Button className="w-full">
              <Stethoscope className="mr-2 h-4 w-4" /> Continue Consultation
            </Button>
          </Link>
        )}
      </div>
      ...
      <Card>
        <CardHeader>
          <CardTitle className="flex items-center gap-2">
            <ClipboardList className="h-5 w-5" /> Visit History
          </CardTitle>
        </CardHeader>
        <CardContent>
          {visits.length === 0 ? (
                ...
                const vitalsParts = consultation
                  ? [
                      consultation.bloodPressure
                        ? `BP ${consultation.bloodPressure}`
                        : null,
                      consultation.oxygenSaturation
                        ? `SpO2 ${consultation.oxygenSaturation}%`
                        : null,
                      consultation.temperature
                        ? `Temp ${consultation.temperature}°F`
                        : null,
                      consultation.pulse
                        ? `Pulse ${consultation.pulse} bpm`
                        : null,
                      consultation.height
                    ...
                    <div className="flex items-center justify-between mb-2 flex-wrap gap-2">
                      <div className="flex items-center gap-3">
                        ...
                        <span className="text-sm text-gray-600">
                          Dr. {visit.doctor?.user?.name} &middot;{" "}
                      ...
                      {canBillThisVisit &&
                        ...
                        consultation && (
                          <Button
                            size="sm"
                            variant="outline"
                            disabled={billingBusyId === consultation.id}
                            onClick={() => generateBill(consultation.id)}
                          >
                            {billingBusyId === consultation.id ? (
